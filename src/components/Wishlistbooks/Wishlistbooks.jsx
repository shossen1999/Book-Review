...
const Wishlistbooks = ({ wishlist }) => {
    const { bookId, publisher, totalPages, yearOfPublishing, image, tags, bookName, author, category, rating } = wishlist;
        ...
        <div className="card flex flex-col lg:flex-row card-side lg:m-0 m-4 bg-base-100 shadow-xl p-8 border-[#282828] border-opacity-[20%] rounded-[16px] border-2">
            ...
            <div className="card-body flex flex-col space-y-2">
                ...
                <div className="flex gap-5 items-center">
                    <div> <p className="px-4 py-1 text-[#328EFF] bg-[#328EFF] bg-opacity-15 rounded-[30px]">Category : {category}</p></div>
                    <div> <p className="px-4 py-1 text-[#FFAC33] bg-[#FFAC33] bg-opacity-15 rounded-[30px]">Rating : {rating} </p></div>
                    <Link to={`/book/${bookId}`}>
